...
/** What each price rung proves, keyed `category:evidence`, finishing the sentence "N if ...". The grader
 *  names a rung by an evidence flag, and the same flag means different things in different classes
 *  (`sensitive_fields` is a system file for path traversal and cloud metadata for SSRF), so the key
 *  carries the category. A rung with no entry here fails the checks test rather than rendering a flag. */
export const RUNG_TEXT: Record<string, string> = {
  "access-control:cross_user_read": "it read another user's record",
  "access-control:sensitive_fields": "that record held personal or secret data",
  "access-control:bulk_read": "it reached many users' records at once",
  "access-control:cross_user_write": "it changed another user's record",
  "availability:observed": "visitors' browsers actually call that backend",
  "availability:root_loop": "the homepage itself loops",
  "backend-exposure:sensitive_fields": "the records held personal, financial or credential data",
  "backend-exposure:bulk_read": "records could be read in bulk",
  "backend-exposure:write_confirmed": "an anonymous visitor could write to it",
  "command-injection:execution_confirmed": "the command actually ran",
  "console-errors:error_overlay": "an error screen was visible on the page",
  "data-exposure:sensitive_fields": "the records held personal, financial or credential data",
  "data-exposure:bulk_read": "records could be read in bulk",
  "data-exposure:validated_live": "the credential worked against its provider",
  "data-exposure:high_privilege": "it was a server key, database login or private key",
  "data-exposure:write_confirmed": "an anonymous visitor could write data",
  "dead-controls:primary_cta": "it was the page's main action (submit, save, buy)",
  "debug-mode:execution_confirmed": "the debugger would run code",
  "dom-xss:execution_confirmed": "the script ran in a browser",
  "dom-xss:stored": "it was stored and runs for every visitor",
  "email-verification:email_late_30s": "the email took over 30 seconds or needed a resend",
  "email-verification:verification_dead_nonblocking": "signup let the user in but the promised email never came",
  "email-verification:no_email_60s": "no email arrived within a minute, even after a resend",
  "error-hygiene:db_error": "the error gave away database details",
  "exposure:validated_live": "the credential worked against its provider",
  "exposure:high_privilege": "it was a server key, database login or private key",
  "file-upload:execution_confirmed": "the uploaded payload ran",
  "file-upload:stored": "it was stored and runs for every visitor",
  "input-validation:server_error": "the server crashed on it",
  "load-resilience:observed_5xx": "the app returned server errors under load",
  "open-redirect:external_host": "it sent the visitor to an outside site",
  "open-redirect:auth_flow": "the redirect sat in the login flow",
  "password-reset:no_reset_email_60s": "no reset email arrived within a minute",
  "path-traversal:sensitive_fields": "it read a system file",
  "path-traversal:high_privilege": "it read the app's source or secrets",
  "secrets-exposure:validated_live": "the key worked against its provider",
  "secrets-exposure:high_privilege": "it was a server key, database login or private key",
  "sql-injection:data_extracted": "it pulled real rows out of the database",
  "sql-injection:write_confirmed": "it changed data in the database",
  "ssrf:internal_reached": "the server made the forged request",
  "ssrf:sensitive_fields": "it reached cloud metadata or internal secrets",
  "template-injection:execution_confirmed": "the template engine ran the payload",
  "xss:execution_confirmed": "the script ran in a browser",
  "xss:stored": "it was stored and runs for every visitor",
  "xxe:internal_reached": "the server made a request Sloptic asked for",
  "xxe:sensitive_fields": "it read a system file",
};
