...
def expire_stale_claims(conn: psycopg.Connection, days: int) -> int:
    ...
    'not_found' never reaches here: an event that does not exist is failed on sight by the checker,
    because no amount of waiting makes a slug appear."""
    rows = conn.execute(
        """
        UPDATE event_claims
           SET status = 'failed',
...
def fail_claim(conn: psycopg.Connection, claim_id: str, reason: str) -> None:
    """Give up on a claim and say why, in a sentence the organizer reads."""
    conn.execute(
        "UPDATE event_claims SET status = 'failed', check_status = 'not_found', "
        "check_detail = left(%s, 2000), checked_at = now() WHERE id = %s AND status = 'pending';",
        (reason, claim_id),
    )
